...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'self'; style-src 'self' https://unpkg.com 'unsafe-inline'; script-src 'self' https://unpkg.com https://cdn.jsdelivr.net 'unsafe-inline';">
...
<body>
  ...
  <script>
    ...
    // PERBAIKAN 2: Format pemanggilan tsParticles diperbaiki
    (async () => {
        await tsParticles.load("tsparticles", { // Menggunakan ID sebagai parameter pertama
            fpsLimit: 60,
            interactivity: {
                events: {
    ...
    async function loadData() {
        ...
        try {
            ...
            const aboutP1 = createAndSetText('p', 'Experienced IT professional...');
            ...
            const createProjectCategory = (title, items) => {
                ...
                items.forEach(item => { ul.appendChild(createAndSetText('li', item)); });
            ...
            certList.innerHTML = '';
            certifications.forEach(cert => { certList.appendChild(createAndSetText('li', cert)); });
            const trainingList = document.getElementById('trainings-list');
            trainingList.innerHTML = '';
            trainings.forEach(training => { trainingList.appendChild(createAndSetText('li', training)); });
        } catch (error) {
    ...
    // ... (Logika popup Anda sama dan sudah benar)
    ...
    popupForm.addEventListener('submit', async (e) => {
        ...
        const formData = {
            ...
            message: document.getElementById('popup-message-text').value
        };
        try {
